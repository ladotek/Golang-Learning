...
type MockDatastore struct {
	Users map[int]User // Field of users and is going to store a map, with key of an int, also store a value of User
}

func (md MockDatastore) GetUser(id int) (User, error) {
	user, ok := md.Users[id] //I check if user exists with an ID (lets say ID=1), this is "comma ok idiom"
	if !ok {
		return User{}, fmt.Errorf("User %v not found", id) // Return error if user with that ID is not found
	}
	return user, nil
}
...
func (md MockDatastore) SaveUser(u User) error {
	_, ok := md.Users[u.ID]
	if ok {
		return fmt.Errorf("User %v already exists", u.ID) // If there is already a user with this ID, return error
	}
	md.Users[u.ID] = u //If there is no user with that ID, for this key (u.UD) , set this value (u)
	return nil         // Return nil, cause there is no error
}
